...
import (
	...
	"github.com/anz-bank/sysl-template/internal/petstore/handlers"
...
func ListenAndServe(cfg *ConfigContainer) error {
	...
	clients, err := petstore.BuildDownstreamClients(&cfg.DefaultConfig)
	if err != nil {
		return err
	}

	// construct the service router
	serviceRouter := petstore.BuildRestHandlerInitialiser(si, restConfig, clients)
